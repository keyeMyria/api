...
class Upload(CommonTest):
    ...
    def test_upload_http(self):
        fname = os.path.join(SITE_PATH, "static/public/other/torvalds-says-linux.ogg")
        ...
        def test_upload(user):
            with open(fname, 'rb') as fp:
                ...
                self.assertEqual(
                    r.status_code, 200,
                    str(user)+" should have access to the uploaded file %s" % url)
                # others must not have access
                self.anon()
                r = self.c.get(url)
                self.assertEqual(
                    r.status_code, 404,
                    "Others must NOT have access to the uploaded file %s" % url)

                # remove an uploaded file (we are testing here)
                filename = os.path.join(FILE_UPLOAD_TEMP_DIR, url[len(MEDIA_URL):])
                return filename
