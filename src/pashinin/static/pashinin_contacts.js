const initPhotoSwipeFromDOM = (gallerySelector) => {
  ...
  const parseThumbnailElements = (htmlTag) => {
    const thumbElements = htmlTag.childNodes;
    ...
    Array.from(thumbElements).filter(el => el.nodeType === 1).forEach((figureEl) => {
      ...
      [linkEl] = Array.from(figureEl.children); // <a> element
  ...
  const onThumbnailsClick = (ev) => {
    ...
    Array.from(clickedGallery.childNodes).filter(el => el.nodeType === 1).forEach((el) => {
